  ...
  try {
    return new Promise((resolve, reject) => {
      ...
      data.foods.forEach((item, index) => {
        const y = doc.y;
        doc.font(normalFont).fontSize(12).fillColor("#111")
          .text(`${index + 1}. ${item.name}${item.variant ? ` (${item.variant})` : ""}`, tableX, y)
          .text(item.quantity.toString(), qtyX, y)
          .text(`${data.currency} ${item.price.toFixed(2)}`, priceX, y)
          .text(`${data.currency} ${item.subtotal.toFixed(2)}`, subtotalX, y);
        ...
        if (item.pizzaSlices) {
          doc.font("Helvetica-Oblique").fontSize(10).fillColor("#555")
            .text(`Slices: ${item.pizzaSlices}`, tableX + 15);
        }
        if (item.selectedPizzas && item.selectedPizzas.length > 0) {
          doc.font("Helvetica-Oblique").fontSize(10).fillColor("#555")
            .text(`Selected Pizzas: ${item.selectedPizzas.join(", ")}`, tableX + 15);
        }

        // Ingredients
        if ((item.ingredients && item.ingredients.length > 0) || (item.extraIngredients && item.extraIngredients.length > 0)) {
          const allIngredients = [
            ...(item.ingredients || []),
            ...(item.extraIngredients || [])
          ];
          const ingredientsText = allIngredients.map((ing) => `${ing.name} (€${ing.price.toFixed(2)})`).join(", ");
          doc.font("Helvetica-Oblique").fontSize(10).fillColor("#555")
            .text(`Ingredients: ${ingredientsText}`, tableX + 15);
          doc.moveDown(0.3);
        }
